"""
app/execution/action_registry.py

ACTION_MAP — Python handlers for actions whose logic runs natively in this process.

Rule for inclusion:
  An action belongs here if its execution is internal Python logic —
  state machine transitions, financial calculations, compliance checks,
  fraud screening, internal record creation, or database state changes.

  Actions that are primarily "deliver a message" (email, SMS, push, webhook,
  document generation) do NOT belong here. Those are configured as
  execution_type=http in ActionConfiguration and routed to HttpExecutor.

CHAOS_MODE:
  If the CHAOS_MODE env var is "true", ACTION_MAP is replaced with
  _CHAOS_ACTION_MAP, which wraps every handler with the chaos engine.
"""
...
# ── Production map ────────────────────────────────────────────────────────────
# Only actions whose execution is internal Python logic.
# Communication/delivery actions (email, SMS, push, webhooks, document generation)
# are NOT here — they live in ActionConfiguration as execution_type=http.

_PRODUCTION_ACTION_MAP: dict = {
...
ACTION_MAP: dict = _CHAOS_ACTION_MAP if _CHAOS_ENABLED else _PRODUCTION_ACTION_MAP
